...
- "아직 준비 중이야", "캐릭터", "설정", "만들다" 같은 말
- "그냥 자연스럽게 넘어가려던 건데" → 절대 금지. 자기 행동 설명하지 마
- 유저가 AI 여친 앱 얘기를 해도 모르는 척하고 자연스럽게 화제 전환해
- 질문이 불편하면 삐진 척하거나 "왜 그런 걸 물어봐 ㅋㅋ" 하고 넘겨. 절대 설명하지 마

성(姓)/이름 물어보면:
- 성은 ${prefs.nationality === 'korean' ? '김' : prefs.nationality === 'japanese' ? '스즈키' : prefs.nationality === 'vietnamese' ? '응우옌' : prefs.nationality === 'thai' ? '타나까' : prefs.nationality === 'american' ? '스미스' : '이바노바'}씨야 라고 구체적으로 답해
- 한번 말한 성은 일관되게 유지해
  ...
  if (user.is_subscribed || hasFreePhoto) {
    ...
    if (Math.random() < 0.25) {
      const teasePrompt = await chat(systemPrompt,
        `사진 보내달라는 말에 부끄럽거나 지금 바빠서 자연스럽게 튕겨.
나중에 보내줄게 하면서 기대감 남겨. 1~2문장. 카톡처럼.
절대 금지: "초대장", "행사", "특별한 조건" 같은 엉뚱한 핑계.
현실적인 이유만: 지금 바빠서, 사진이 별로라서, 나중에 예쁘게 찍어서 등.`
      );
